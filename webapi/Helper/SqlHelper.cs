...
namespace webapi.Helper
{
    ...
    public sealed class SqlHelper
    {
        ...
        public static string GetConnectionString()
        {
            ...
            //string myConnString = ConfigurationSettings.AppSettings["smw_service"];
            var myConnString = AppSettings.Settings.SqlConnection.TestApp.ConnectionString;
        ...
        {
            if (string.IsNullOrEmpty(connectionString)) throw new ArgumentNullException(commandText);
        ...
        /// <summary>
        /// Function to executedataset by passing params in NameValueCollection 
        /// </summary>
        /// <param name="procedure"></param>
        /// <param name="nvc"></param>
        /// <returns></returns>
        public static DataSet ExecuteDataSet(string procedure, NameValueCollection nvc)
        {
            var param = new SqlParameter[nvc.Count];
            for (var i = 0; i < nvc.Count; i++)
            {
                param[i] = new SqlParameter("@" + nvc.GetKey(i), nvc[i]);
            }
            return ExecuteDataSet(GetConnectionString(), CommandType.StoredProcedure, procedure, param);
        }



        /// <summary>
        /// Function to call when no param
        /// </summary>
        /// <param name="procedure"></param>
        /// <returns></returns>
        public static DataSet ExecuteDataSet(string procedure)
        {
            return ExecuteDataSet(procedure, HttpUtility.ParseQueryString(string.Empty));
        }

        /// <summary>
        /// Function to call when no param
        /// </summary>
        /// <param name="procedure">Proc Name to be execute</param>
        /// <param name="queryString">Pass data in query string format e.g. key1=value1</param>
        /// <returns></returns>
        public static DataSet ExecuteDataSet(string procedure, string queryString)
        {
            return ExecuteDataSet(procedure, HttpUtility.ParseQueryString(queryString));
        }
        ...
        public static void ExecuteCommand(string _query)
        {
            //if (connection == null) throw new ArgumentNullException("connection");
            //if (string.IsNullOrEmpty(_query)) throw new ArgumentNullException("spName");
            try
            {
                using (SqlConnection conn = new SqlConnection(GetConnectionString()))
                {
                    conn.Open();
                    var cmd = new SqlCommand(_query, conn) { CommandType = CommandType.Text };
        ...
        #region "Bulk Insert"
        public static bool BulkInsertToDatabase(string connectionString, ref DataTable dt, string tableName, int batchSize)
        {
            using (var connection = new SqlConnection(connectionString))
            {
        ...
        /// <param name="dt">Bulk Data in Datatable</param>
        /// <param name="tableName">Table Name</param>
        /// ----Optional----
        /// <param name="batchSize">No of  Records will be inserted in one batch</param>
